...
import { Merge, merge } from "lume/core/utils/object.ts";
...
export interface Options {
  ...
  /**
   * The folder where font files live — the same convention used by
   * `google_fonts` and `katex` for the fonts they generate. Fonts placed
   * here (locally, or mapped remotely with `site.remote()`) are picked up
   * automatically, local or remote, added or not.
   * @default `site.options.fontsFolder` (`/fonts` by default)
   */
  fontsFolder?: string;
  /**
   * Explicit font files to load individually, as paths (relative to `src`)
   * or remote URLs. Use this for fonts that don't live in `fontsFolder`.
   */
  fonts?: string[];
  /**
...
export class TypstEngine implements Engine {
  ...
  readonly #fontsFolder: string;
  #compiler: Promise<NodeCompilerType> | null = null;

  constructor(site: Site, options: ResolvedOptions, fontsFolder: string) {
    this.#site = site;
  ...
  /**
   * Loads font blobs from three sources, all Lume-virtual-fs aware so they
   * work for local and remote-mapped fonts alike:
   * - `options.fonts`: explicit, individually listed files/URLs.
   * - `site.pages` / `site.files` inside `fontsFolder`: fonts that exist
   * only as already-built pages (e.g. generated by `google_fonts`).
   * - `site.fs.entries` inside `fontsFolder`: raw font files that were
   * never promoted to a page/file (no `site.add()` needed).
   */
  async #collectFontBlobs(): Promise<Uint8Array[]> {
    ...
    for (const file of [...this.#site.pages, ...this.#site.files]) {
      ...
      if (
        !url || !url.startsWith(this.#fontsFolder) ||
    ...
    for (const [path, entry] of this.#site.fs.entries) {
      if (
        entry.type !== "file" || // Safety check to ignore directories with font extensions
        seen.has(path) ||
        !path.startsWith(this.#fontsFolder) ||
        !FONT_EXTS.has(posix.extname(path))
      ) {
        continue;
      }
      ...
      try {
        ...
        blobs.push(content as Uint8Array);
      } catch (error) {
...
export function typst(userOptions?: Options) {
  ...
  return (site: Site) => {
    const fontsFolder = posix.join(
      "/",
      options.fontsFolder ||
        (site.options as Record<string, unknown>).fontsFolder as string ||
        "/fonts",
    );

    const engine = new TypstEngine(site, options, fontsFolder);
